...
rl.on('line', (line) => {
    N--;
    db.push({val: parseInt(line, 10), idx: 8 - N});
   if (N === 0) {
        db.sort((a, b) => a.val - b.val);
        db.splice(0, 3);
        console.log(db.map(_ => _.val).reduce((_, i) => _ + i));
        console.log(db.map(_ => _.idx).sort((a, b) => a - b).join(' '));
        process.exit();
   }
});
...
1 초	128 MB	5797	4220	3912	75.187%
문제
