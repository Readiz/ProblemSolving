#include <stdio.h>
    ...
    inline void testInit(char fbasename[]) {
        freopen("./2042_input.txt", "rt", stdin); 
    }
...
int N, M, K;
ll db[1'000'000];
int treeIdxFromDBIdx[1'000'000]; // Leaf Node를 생성하면서 Idx를 저장해두는 배열
...
MAIN_START
    ...
    for (int i = 0; i < M + K; i++) {
        ...
        if (cmd == 1) { // Update
            db[a - 1] = b;
            updateSegTree(treeIdxFromDBIdx[a - 1]); // a 번째 값을 업데이트
        } else { // Query
