...
ll bucket[400][256];
ll bucketSum[400];
pair<ll, ll> bucketLazy[400]; // ax * b => {a, b}
...
inline void handleLazy(int b) {
    for(register int i = 0; i < 256; ++i) {
        bucket[b][i] = reduce(bucket[b][i] * bucketLazy[b].first + bucketLazy[b].second);
    }
    bucketLazy[b] = {1, 0};
}
...
inline void updateRange2(int l, int r, ll v) {
    ...
    if (sb == eb) {
        ...
        // 2. lazy
        for(register int i = sb + ((l&BMASK) != 0); i < eb; ++i) {
            bucketLazy[i] = {reduce(bucketLazy[i].first * v), reduce(bucketLazy[i].second * v)};
...
int main() {
    for(int i = 0; i < 400; ++i) bucketLazy[i] = {1, 0};
