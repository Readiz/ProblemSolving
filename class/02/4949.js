...
while (true) {
    ...
    for (let i = 0; i < line.length; i++) {
        if (['(', '['].indexOf(line[i]) > -1) last += line[i];
        if ([']', ')'].indexOf(line[i]) > -1) {
            if (last.length > 0) {
                if (last[last.length - 1] === '(' && line[i] === ')') {
                    last = last.slice(0, last.length - 1);
                } else if (last[last.length - 1] === '[' && line[i] === ']') {
                    last = last.slice(0, last.length - 1);
                } else {
                    flagYes = false;
                    break;
    ...
    if (!flagYes) console.log('no');
    else if (last === '') console.log('yes');
    else console.log('no');
}
...
1 초	128 MB	27059	8998	7305	33.301%
문제
