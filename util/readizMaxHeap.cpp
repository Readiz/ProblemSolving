// 최대힙 구현. 빈 공간은 MINF 데이터로 차 있어야 함.
// Push / Pop: log(n) 로 보장.
...
int popHeap() {
    ...
    while (p < sizeHeap) {
        int lChild = (p + 1) * 2 - 1;
        int rChild = (p + 1) * 2;
        if (Heap[p] < Heap[lChild] || Heap[p] < Heap[rChild]) { // 자식이 더 크면 바꿔야 하는 케이스임
            if (Heap[lChild] > Heap[rChild]) { // 더 큰 자식이랑 바꾼다.
        ...
        } else { // 더 이상 자식까지 갈 필요 없음 (부모가 두 자식보다 더 크다.)
            break;
        }
    }
    return r;
...
void pushHeap(int num) {
    ...
    while (p > 0) {
        ...
        if (Heap[p] > Heap[parent]) { // 부모가 더 커야 함
